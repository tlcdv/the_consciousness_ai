...
2.  **Binding Validation (empirical record, chapter closed):** The 2026-02-21 3-condition synthetic test demonstrated phi monotonicity with binding strength on a controlled stimulus. In-training tests of the pre-registered Phi-1 prediction (phi correlates r > 0.4 with binding sync_R) across **9 runs spanning 4 architectures and 2 phi formulations all FAILED** on the pre-registered positive-direction threshold: 2026-05-14 5-variant AKOrN ablation campaign (best r=+0.089); 2026-05-16 AKOrN+RIIU (r=+0.075, transient single-seed +0.267 that does not replicate); 2026-05-17 substrate probe (NO WINNER); 2026-05-17/18 AKOrN+A+C+D retest (pyphi r=-0.062, RIIU r=-0.005 NS); 2026-05-19 AKOrN+A+B+C+D Phase B (pyphi r=+0.008, RIIU r=-0.007 NS); **2026-05-24 KomplexNet+A+C+D Phase B-alt (pyphi r=+0.011 NS, RIIU r=-0.1116 p<10^-100, substantively significant NEGATIVE correlation)** ([docs/results/phi1_phaseBalt_2026_05_24.md](docs/results/phi1_phaseBalt_2026_05_24.md)). The KomplexNet result is the first substantively significant phi-binding correlation in the campaign and reveals a mechanistic finding: **for this class of architecture, oscillatory binding and integrated information (per RIIU's SVD-residual formulation) are inversely related, not coupled**. When phases align, module content factors cluster near +1, producing low representational variance and low RIIU phi; when phases desync, content factors span [-1, +1] and produce high RIIU phi. The pre-registered escalation chain (sections 10-11-12 of `docs/preregistered_predictions.md`) has been exhausted. The project proceeds to Phase 5 (Dynamic Self-Representation) with the Phi-1 strong-emergence claim removed from public claims.
