...
class TestEmotionalReinforcement(unittest.TestCase):
    ...
    def test_meta_learning_adaptation(self):
        ...
        for i in range(12):
            ...
            self.rl_core.step(
                state=state, action=action, raw_reward=0.5,
                next_state=next_state, done=(i == 11),
    ...
    def test_full_interaction_loop(self):
        ...
        for step in range(10):
            ...
            result = self.rl_core.step(
                ...
                next_state=next_state, done=(step == 9),
                emotion_state=emotion, attention_level=0.7,
            )
            total_shaped += result['shaped_reward']
            state = next_state

        self.assertEqual(len(self.rl_core.rollout_buffer), 10)
