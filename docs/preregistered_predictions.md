...
## 1b. Causal Emergence 2.0 (CE 2.0) Predictions

**Framework:** Hoel (2025, arXiv:2503.13395v3), the SVD heuristic in Supplementary S3. CE 2.0 grounds macroscale causation in the causal primitives (determinism plus specificity), estimated from the singular values of a row-stochastic TPM, and drops the log2(n) size term that EI carries. It supersedes the EI predictions above as the primary causal-emergence instrument.

**Date of pre-registration:** 2026-07-23 (CE2-1, CE2-3, CE2-4). CE2-2 is deferred: its band is read off a single-seed calibration pilot and registered here before the multi-seed test, so no run both sets and tests it.

**Measurement method:** `compute_and_log_ce2()` in `scripts/training/metrics_logger.py`. `ce2_gates` and `ce2_workspace` are sigma_2 minus gamma* on the gate (243-state) and workspace (8-state) TPMs, using the same discretization EI uses. `ce2_ratio` is workspace over gates. `ce2_rssm` is sigma_2 minus gamma* on the RSSM discrete-latent 32x32 class-transition TPM.

**Honesty note:** applying this coarse-grain-of-Markov-chains heuristic to a trained neural latent is exploratory and is not validated by the source paper. Every headline value requires >= 3-seed replication. CE 2.0 causal emergence is one indicator, not proof of consciousness.

### Prediction CE2-1: Onset of workspace causal emergence

**Claim:** `ce2_workspace > ce2_gates` will first occur between episode 500 and episode 2000 of Dark Room training, sustained across a stretch of episodes rather than a single-episode blip.

**Rationale:** Near-initialization weights give near-uniform TPMs, for which CE 2.0 reads approximately zero at all levels. Once training makes the workspace a genuine coarse-graining of gate noise, its sigma_2 minus gamma* should exceed the gates'. This is the direct GNW claim that the workspace is a causally emergent macroscale over its micro-level gates.

**Falsification:** If `ce2_workspace <= ce2_gates` for all episodes up to 5000, the workspace is not causally emergent and the architecture needs revision.

### Prediction CE2-2: Magnitude stabilization (deferred; band set from the pilot)

**Claim:** After convergence (episodes 3000 to 5000), `ce2_ratio` stabilizes within a band [TBD] rather than oscillating or diverging.

**Rationale:** EI-2's band (1.1 to 3.0) was calibrated for a metric that includes the log2(n) size term. CE 2.0 removes it, so those numbers do not transfer. The band is read off the single-seed calibration pilot and registered before the multi-seed test. Fixing a number now would be unsupported.

**Falsification:** Defined once the band is fixed.

### Prediction CE2-3: CE 2.0 and task performance correlation

**Claim:** Episodes where `ce2_workspace > ce2_gates` will correlate (Pearson r > 0.3) with episodes where cumulative reward is above the running median.

**Rationale:** If macroscale causal power is functionally relevant, it should correspond to better task performance. A system that is causally emergent but performs no better has an epiphenomenal workspace.

**Falsification:** If the correlation is negative or near zero (|r| < 0.1) across 5000 episodes, workspace emergence is not contributing to behavior.

### Prediction CE2-4: RSSM latent collapse tracking

**Claim:** In `--rssm-latent-mode discrete`, `ce2_rssm` stays low and does not rise while the perception collapse persists (the discrete latent decodes stimulus identity at chance, per the 2026-06 collapse-locus probes).

**Rationale:** A collapsed latent produces a near-degenerate transition TPM, for which sigma_2 minus gamma* is small. This ties CE 2.0 to a known, independently measured failure and validates that it reads real causal structure.

**Falsification:** If `ce2_rssm` rises substantially while identity decodability stays at chance, CE 2.0 is not tracking the causal structure it is assumed to track.
