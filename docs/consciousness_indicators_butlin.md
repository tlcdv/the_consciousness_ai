...
| RPT-2: organized, integrated perceptual representations | Topographic map + inverse-effectiveness fusion, hierarchical capsule composition, AKOrN oscillatory binding | PARTIAL, **rationale corrected 2026-09-13** (`results/rpt2_evidence_audit_2026_09.md`). The ORGANIZED half is measured at 3 seeds: `obs_map` decodes 6-class `sample_shape` at ~1.0, `kl_map` at 0.84 / 0.71 / 0.76, the 256-D broadcast at 0.76 / 0.69 / 0.77. The INTEGRATED half has NEVER been validly measured. **The previous rationale, "the in-training phi-binding coupling (Phi-1) FAILED across 9 runs", is withdrawn.** The pre-registration requires non-degenerate variances before a negative may be accepted (`preregistered_predictions.md` line 599; floors `phi_std` > 0.01 and `sync_R_std` > 0.02), and 0 of the 9 runs met them: every `sync_R_std` is at or below 0.019. The individual run documents recorded the correct verdict, RE-RUN, at the time; this summary line drifted from them. The cause was established later: `reset_state` at `oscillatory_binding.py:149` is never called from the training loop, so the oscillators converge and `sync_R` becomes the mean of the separately degenerate bids, which pins its variance below the floor in any run of this architecture. That rationale also demanded a measured training-time signature, which the IMPLEMENTED definition settled 2026-08-11 states is NOT necessary. Status stays PARTIAL: withdrawing invalid negative evidence leaves no evidence, not a positive, and mechanism presence is not sufficient. Whether the ORGANIZED half alone counts toward this indicator is an open owner decision |
...
| GWT-2: limited-capacity workspace with a bottleneck + selective attention | GNW ignition threshold + AKOrN binding selecting the winner; the 256-D broadcast is the low-dimensional bottleneck | PARTIAL: the bottleneck half holds and is by design (the 2026-06-02 localization, the source of the control-vs-integration trade reading #2 accepts). The SELECTIVE ATTENTION half is measured false. In a live run at 3 seeds the winner never changes: vision takes 99.0 to 99.8 percent of steps and 2000 of 2000 in the final 2000 steps of every seed, with four of five bids emitting a literal constant (`results/workspace_bids_live_2026_08.md`). Two earlier lines of evidence agree: the ignition gate is saturated at 99.79 to 100 percent of steps, and sync_R is perception-invariant **Repair attempt CLOSED 2026-09-13** (`results/bid_counterfactual_2026_09.md`). Offline replay of 6 vision-bid reductions through the real settle loop, on 3 independently trained checkpoints: the saturation IS fixable by self-normalization (about 1200 distinct bid values on every checkpoint, no tuned constant), and fixing it produces NO competition. 0 of 24 candidate/opponent combinations give a changing winner at all 3 checkpoints. A fixed divisor is disqualified outright: the pre-`tanh` KL scale varies 8,079x across checkpoints, so `tanh(kl/N)` yields 484 distinct values on one and 2 on another. **The blocker is NOT the bid formula.** It is that four of five bids are literal constants, and that ignition requires `input_energy >= EMA(input_energy)`, so lowering the vision bid SILENCES the workspace instead of handing the win to the runner-up. |
