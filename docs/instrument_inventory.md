...
retired, and 6 are not running.** As of 2026-09-13 the PCI gate question is SETTLED and
is not an instrument problem: the gate's causal response saturates at 0.55x its own
fluctuation, so no impulse can make it detectable. Seven reading rules stated.
...
|---|---|---|
| **PCI** `perturbational_complexity.py` | **THE GATE IS UNREACHABLE BY PERTURBATION AT ANY MAGNITUDE** (`pci_gate_saturation_2026_09.md`, 2026-09-13). The floor-independent measurement: the gate's causal response is 0.26 / 0.00 / 0.01 times its OWN spontaneous fluctuation across three trained-gate checkpoints, where 3 is needed to register, while the control clears it at 45.6 / 6.4 / 6.7. A magnitude sweep on `gate3_s42` shows the gate SATURATES rather than scales: ratio 0.27 at 1e3, 0.51 at 1e4, 0.55 at 1e5, so 100x the impulse buys 2x the response and 3.0 is unreachable. The 2026-08 linearity was a property of the rssm, not the gate. This supersedes the variance-floor question, which was secondary: the obstruction is a ratio, not a threshold. The per-site floor attempt also FAILED its own noise control (non-zero from pure noise in 29 of 40 seeds), and is kept default-off with the defect pinned by a test. The instrument itself is sound: control discriminates everywhere, determinism 0.0e+00 throughout, and `pci_trained_2026_08.md` re-checked at a 1e-6 floor still reads 0.0000. Seven reading rules now stated. | Find the saturation in the path from `h_state` to the gate; `sensory_tectum.py:456` is a documented tanh and the first place to look. Repeat the sweep on seeds 43 and 44. PCI's readability at the GATE is now an architecture question, not an instrument question. |
