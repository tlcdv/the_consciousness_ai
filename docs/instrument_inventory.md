...
retired, and 6 are not running.** PCI met the FIRST HALF of its falsifier on 2026-09-13
at three trained gates. It stays UNPROVEN on the reading rules, which got harder.
...
|---|---|---|
| **PCI** `perturbational_complexity.py` | The strongest entry, and the FIRST HALF of its falsifier is now met (`pci_three_trained_gates_2026_09.md`, 2026-09-13). Three checkpoints trained for this purpose at seeds 42/43/44, each with a TRAINED gate, 100 episodes, `--rssm-latent-mode continuous --capsule-workspace-source all_levels`. The gate reads exactly 0.0000 at all three while the control reads 0.0471 / 0.1631 / 0.0626, determinism 0.0e+00 throughout, every row marked `weights=trained`. So the gate is causally inert across independently trained networks, not just on one checkpoint. NOTE the earlier 2026-09-12 attempt is VOID: its three extra checkpoints had randomly initialised gates. What now blocks TRUSTED is the reading rules, and this study made them harder: the control spans 3.5x across seeds of ONE configuration (0.0471 to 0.1631), so it cannot serve as a fixed reference. `pci_casali` diverges with sparsity exactly as the module predicts, up to 35.4x at active_fraction 0.0040, and exceeds the human 0.31 cutoff at sites whose local reading is far below it. Cite `pci`, never `pci_casali`, neither against the human scale. A broadcast reading of 0.0232 at seed 42 only is a HYPOTHESIS at 1 of 3 seeds, not a result. | State the reading rules, given a control that moves 3.5x across seeds. Then probe a checkpoint from a DIFFERENT training configuration: all three share one. |
