...
retired, and 6 are not running.** As of 2026-09-13 the PCI gate question is NOT settled.
An earlier verdict the same day said the gate saturates and that the variance floor was
the wrong question; it is RETRACTED. The gate's response is stable across probe seeds
while its BASELINE moves by 84x, so a reading is usable on some probe seeds and not
others. Seven reading rules stated, and rules 1 and 2 restored.
...
|---|---|---|
| **PCI** `perturbational_complexity.py` | **THE ATTENUATION IS AT ONE STAGE, AND THE GATE DOES RESPOND ON SOME PROBE SEEDS** (`pci_gate_attenuation_2026_09.md`, 2026-09-13). Instrumenting every stage between the impulse and the gate: `h_state` scales exactly 100x with a 100x impulse, and `tectum_content`, `broadcast`, `gate_in` and `enriched` are IDENTICAL to every printed digit at every magnitude in 5 runs, so the workspace, the reentrant settle and the gate input add ZERO attenuation. The whole nonlinearity is the tectum forward that turns `h_state` into `tectum_content`. `vision_bid` is 1.0000000000 with sd 0.0e+00 and a clean-vs-perturbed difference of exactly zero, so the `tanh` at `sensory_tectum.py:456` transmits nothing; it was the stated first suspect and it is NOT the cause. The gate sigmoids sit at 0.485 to 0.498, their linear region, so they are cleared too. The obstruction is the DENOMINATOR: on one checkpoint the gate response varies 2.1x across five probe seeds while its baseline sd varies 84x (2.520e-06 to 2.120e-04), and the two seeds that register at a 1e-6 floor are exactly the two with a quiet baseline. The default 1e-4 floor sits 40x above the quiet baseline and silences an 11.53-sigma detection, which VINDICATES reading rules 1 and 2. Located but not explained: `tectum_content` baseline sd is constant at 1.084e-04 across probe seeds while `broadcast` reads 1.084e-04 or 5.001e-03, so something between them injects seed-dependent variance. **RETRACTED: `pci_gate_saturation_2026_09.md`**, whose 0.26 and 0.55 were undeclared medians across probe seeds disagreeing by 84x, whose "3.0 is unreachable" is false (5 of 15 cells exceed 3), and which drew its conclusion from two magnitudes its own rule 7 disqualifies. | Build the null over PROBE SEEDS and on the BASELINE, not the response: about 20 zero-impulse runs, recording the gate baseline sd distribution. Then find what injects variance into the broadcast (three untested candidates named in the verdict), and locate the compression inside `sensory_tectum.forward`. |
