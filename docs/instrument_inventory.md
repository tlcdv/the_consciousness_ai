...
retired, and 6 are not running.** PCI has three trained-gate checkpoints and stated
reading rules as of 2026-09-13, and stays UNPROVEN: its default variance floor sits
ABOVE the gate substrate, so every gate-level zero read at the default is unreadable.
...
|---|---|---|
| **PCI** `perturbational_complexity.py` | BOTH halves of the falsifier now have work against them, and the instrument is more constrained than it looked. Reading rules are stated in `pci_reading_rules_2026_09.md`. The decisive finding: `DEFAULT_VAR_FLOOR` is 1e-4 and the gate nodes fluctuate at 4.1e-05 to 6.9e-05, so AT THE DEFAULT THE GATE CANNOT REGISTER A RESPONSE AT ANY SIZE. At a floor of 1e-6 the gate reads 0.0439 on `gate3_s42` (2 trials of 5 at 0.1097) while `gate3_s43` and `gate3_s44` stay at exactly 0.0000 with a raw response of 1.192e-07, so lowering the floor did not manufacture a signal where there is none. Three trained-gate checkpoints exist now (seeds 42/43/44, `pci_three_trained_gates_2026_09.md`), controls 0.0471 / 0.1631 / 0.0626, determinism 0.0e+00, every row `weights=trained`. The control spans 3.5x across seeds of ONE configuration, so absolute PCI is NOT comparable across checkpoints. `pci_casali` inflates with sparsity up to 35.4x and exceeds the human 0.31 cutoff at noise sites. | Decide whether the five reading rules are sufficient for TRUSTED, given that rule 3 forbids cross-checkpoint comparison. Settle whether `gate3_s42`'s 2-of-5 response replicates at more seeds. Consider a PER-SITE floor so rule 2 is automatic. Re-examine every earlier verdict that read a gate-level zero at the default floor. |
