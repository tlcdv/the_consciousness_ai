...
retired, and 6 are not running.** A 2026-09-12 attempt to meet PCI's falsifier FAILED:
only one checkpoint on disk has a trained gate, so meeting it needs new training runs.
...
|---|---|---|
| **PCI** `perturbational_complexity.py` | Still the strongest entry, and its falsifier is NOT met. A 2026-09-12 attempt to run it on four checkpoints FAILED its own design: three of the four have no `tectum.gate.pt`, so their gate ran randomly initialised and their gate readings say nothing about a trained system (`pci_multi_checkpoint_2026_09.md`). Exactly ONE checkpoint in `runs/` has a trained gate, `gate_ckpt_s42`, because gate weights were not saved before 2026-08-11. So the only valid gate reading remains the published 0.0000, reproduced exactly. What the attempt did establish: the rssm CONTROL is stable across four independently trained TECTA, 0.0552 to 0.0657, and the predicted `pci_casali` divergence is real at size, 5.6x at `capfix_seed44`'s control (0.0552 against 0.3065, active_fraction 0.0274) and 1000x at its broadcast. The module docstring already predicted that divergence; the numbers are what is new. One checkpoint in four lands on the published human 0.31 cutoff under Casali. Cite `pci`, never `pci_casali`, and neither against the human scale. | Train at least two more checkpoints with the current code, which saves `tectum.gate.pt`, then re-run. This is a TRAINING job, not a read-only probe. Then state the reading rules, including which normalization is authoritative. |
