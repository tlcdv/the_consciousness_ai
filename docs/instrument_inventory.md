...
retired, and 6 are not running.** As of 2026-09-13 PCI has its READING RULE, which is
half of its falsifier: the gate baseline is bimodal across probe seeds with a 42x to 60x
empty gap, so usable seeds are selectable. On `gate3_s42` the gate responds on 16 of 20
probe seeds; on the other two trained checkpoints it responds on 0 of 20, at float32
resolution. One checkpoint of three, so this is NOT an architecture claim. Eight reading
rules stated. PCI stays UNPROVEN: nothing yet shows a gate PCI carries stimulus content.
...
|---|---|---|
| **PCI** `perturbational_complexity.py` | **THE READING RULE EXISTS, AND THE GATE RESPONDS ON ONE CHECKPOINT OF THREE** (`pci_probe_seed_null_2026_09.md`, 2026-09-13). 20 probe seeds on each trained-gate checkpoint, magnitude 1000, floor 1e-6, determinism 0.000e+00 and every site `[trained]` on every run. The gate baseline is BIMODAL across probe seeds with an EMPTY multiplicative gap of 42.5x to 60.3x, and the four noisy seeds are the same four (43, 44, 45, 53) on independently trained checkpoints, so the regime belongs to the rollout and not the weights. That gap is rule 8. On `gate3_s42` the gate registers on 16 of 20 seeds at 6.87 to 24.42 times its own baseline, mean PCI 0.1070 against a control of 0.1640. On `gate3_s43` and `gate3_s44` it registers on 0 of 20, responses 0 to 2.384e-07, which is 0 to 8 ulp of float32, so those two are genuinely unreachable and not floor artefacts. The default 1e-4 floor reads 0.0000 on all 20 `gate3_s42` seeds because the 16 quiet ones sit 40x BELOW it, which confirms rules 1 and 2. Hard limit found: gate PCI is coarsely quantized, lz in {2,4,5,6} on a 5x60 matrix, so only four PCI values exist and no floor changes that. Attenuation located earlier the same day (`pci_gate_attenuation_2026_09.md`): the whole nonlinearity is the tectum forward from `h_state` to `tectum_content`; `vision_bid` is 1.0000000000 with zero response so `sensory_tectum.py:456` transmits nothing, and the gate sigmoids sit at 0.485 to 0.498, their linear region. **RETRACTED: `pci_gate_saturation_2026_09.md`.** | Content-test the gate PCI on `gate3_s42`, the only site with a replicated response, using the same eta-squared permutation null used elsewhere. A response that replicates is not yet a response that means anything, and that clause is what stands between PCI and TRUSTED. Then ask why 1 checkpoint of 3 has a reachable gate. |
